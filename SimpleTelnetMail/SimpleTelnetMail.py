...
class TelnetMail :
    ...
    def __init__ (self, host : str, port : int = None, from_ : str = None, to : list = None, message : str = None, ehlo : str = "SimpleTelnetMail", pseudo : str = "", ssl : bool = True, debug : int = 0, **kwargs) :
...
def advanced_usage () :
    client = TelnetMail("my.server.com", port = 587, from_ = "my.address@domain.com", to = ["receiver1@domain.com", "receiver2@domain.com"], message = "Secret and secure email with Telnet.", ehlo = "H4CK3R", pseudo = "Mr_X", ssl = True, debug = 4, Subject="Secret Email", Date="Sat, 19 Dec 2020 01:02:03 -0000", MIME_Version="1.0", Encrypted="ROT13", Fake="Fake hearder", Sender="PSEUDO <test@free.fr>", Comments="My comment", Keywords="Email, Secret", Expires="Sat, 25 Dec 2021 05:35:23 -0000", Language="en-EN, it-IT", Importance="hight", Priority="urgent", Sensibility="Company-Confidential", From="PSEUDO <test@free.fr>", To="receiver1@domain.com,receiver2@domain.com", Content_Type="text/plain; charset=us-ascii", Content_Transfer_Encoding="quoted-printable")
    client.send_mail()
...
def main () :
    ...
    help_ = False
    arguments = { "host" : None, "from" : None, "to" : None, "message" : None, "port" : 25, "ehlo" : "SimpleTelnetMail", "pseudo" : " ", "debug" : 0, "ssl" : False }
    headers = {}

    optlist, args = getopt(argv[1:], "H:p:f:t:e:m:P:d:s", ["host=", "port=", "from=", "to=", "ehlo=", "message=", "pseudo=", "debug=", "ssl"])
    ...
    for argument in optlist :
        if argument[0] == "--host" or argument[0] == "-H" :
            ...
            try :
                ...
                print("ArgumentError: --port value or -p value must be an integer. Port is set to 25.")
            ...
            try :
                ...
                print("ArgumentError: --debug value or -d value must be an integer. Debug is set to 0.")
        else:
            breakpoint()
            help_ = True
            break
